...
def skip_file_lines(data, skip):
    ...
    if (skip[0] + skip[1]) >= len(data.splitlines()):
        logger.error(
            "The total number of lines skipped can't be more the total number of line in the text"
        )
    logger.debug(
    ...
    return "\n".join(
        data.splitlines()[int(skip[0]): len(data.splitlines()) - int(skip[1])]
    )
...
if __name__ == "__main__":
    ...
    if not args.histogram and args.output:
        logger.warning(
            "The --histogram argument is required for the --output argument '{args.output}' file will not be created nor displayed"
        )
    charcount(
        args.file_path,
        skip_lines,
