...
def skip_file(data,skip):
    ...
    if (len(data.splitlines()) - skip[0]) <= 0:
        logger.error("skip_top can\'t be more then the total number of lines in the text")
    if (len(data.splitlines()) - skip[1]) <= 0:
        logger.error("skip_bot can\'t be more then the total number of lines in the text")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--char-numb","-n",  action='store_true',   help= "show characters total count instead of frequencies")
    parser.add_argument('--gutenberg',"-g",  action='store_true',   help= "detect starting and ending line of gutenberg project ebooks and automaticly skips preamble and license of the book")
    parser.add_argument('--histogram',"-H",  action='store_true',   help= "plot a histogram of the character frequencies on screen")
    parser.add_argument("--output","-o", nargs= '?',    const="Histogram.pdf",  metavar="OUTPUT_PATH",  default= False, help= "Save the histogram in OUTPUT_PATH, default path is 'Histogram.pdf'. --histogram is required to save the output file")
    parser.add_argument("--skip-lines","-sl", nargs= 2,   metavar=('SKIP_TOP','SKIP_BOT'),    default= (0,0), help= "skip SKIP_TOP line(s) from the top of the text and SKIP_BOT from the bottom of the text")
