...
public:
    VectorXYZT() {}
    VectorXYZT(T px, T py, T pz, T e) : m_px(px), m_py(py), m_pz(pz), m_e(e) {}
    // getters
...
public:
    VectorPtEtaPhiMass() {}
    VectorPtEtaPhiMass(T pt, T eta, T phi, T m) : m_pt(pt), m_eta(eta), m_phi(phi), m_m(m) {}
    // getters
    ...
    operator VectorXYZT<T> const()
    {
        return VectorXYZT(px(),py(),pz(),e());
    }
    // setters
